...
1. Install `goreman`: `go install github.com/mattn/goreman@latest`
2. Start dependencies: `docker-compose -f _local/docker-compose-deps.yaml up -d`
3. Initialize databases: `scripts/localdev-init.sh`
4. Start Armada components: `goreman start`
...
3. Press `CTRL-C` to stop skaffold and do a cleanup.

**Note**: The Lookout UI is served by the Lookout backend on port 8089. In Goreman mode, you can also run
the frontend dev server separately on port 3000 for hot-reload during UI development.
