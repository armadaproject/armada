...
func runDescribeNode(cmd *cobra.Command, args []string) error {
	...
	// 1) explicit --run-id
	// 2) explicit --job-id
	// 3) positional arg (treated as job-id by default)
	runID := describeRunID
	jobID := describeJobID
	...
	if runID != "" {
		resp, err = client.DescribeNodeByJobRun(ctx, &introspectionapi.DescribeNodeByJobRunRequest{
			RunId: runID,
		})
	} else {
		resp, err = client.DescribeNodeByJobId(ctx, &introspectionapi.DescribeNodeByJobIdRequest{
			JobId: jobID,
		})
	}
	if err != nil {
