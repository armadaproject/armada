...
func preemptNodeCmd() *cobra.Command {
	...
	cmd.Flags().StringSliceP("priority-classes", "p", []string{}, "Preempt jobs on node for specified executor matching the specified priority classes. Provided priority classes should be comma separated, as in the following example: armada-default,armada-preemptible.")
	return cmd
}
