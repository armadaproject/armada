...
executor: export KUBECONFIG=.kube/external/regatta/regatta-1 && ${GO_BIN:-go} build -gcflags="all=-N -l" -o ./dist/armada-executor ./cmd/executor/main.go && ./dist/armada-executor --config ./cmd/regatta/config/armada/executor/two-cluster/config-regatta.yaml
executor2: export KUBECONFIG=.kube/external/regatta/regatta-2 && ${GO_BIN:-go} build -gcflags="all=-N -l" -o ./dist/armada-executor2 ./cmd/executor/main.go && ./dist/armada-executor2 --config ./cmd/regatta/config/armada/executor/two-cluster/config-regatta-2.yaml
