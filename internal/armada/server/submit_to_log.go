...
func PulsarSequenceFromApiEvent(msg *api.EventMessage) (sequence *armadaevents.EventSequence, err error) {
	...
	case *api.EventMessage_LeaseReturned:
		...
		jobId, _, err := parseJobRunIds(m.LeaseReturned.JobId)
		if err != nil {
			return nil, err
		}
	...
	case *api.EventMessage_Pending:
		...
		jobId, _, err := parseJobRunIds(m.Pending.JobId)
		if err != nil {
			return nil, err
		}
	...
	case *api.EventMessage_Running:
		...
		jobId, _, err := parseJobRunIds(m.Running.JobId)
		if err != nil {
			return nil, err
		}
	...
	case *api.EventMessage_Succeeded:
		...
		jobId, _, err := parseJobRunIds(m.Succeeded.JobId)
		if err != nil {
			return nil, err
		}
	...
	case *api.EventMessage_IngressInfo:
		...
		jobId, _, err := parseJobRunIds(m.IngressInfo.JobId)
		if err != nil {
			return nil, err
		}
