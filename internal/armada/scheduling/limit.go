...
// maximumJobSizeBytes          - This should represent a value bigger than the maximum job size
//                                It is used as a threshold to determine when adding more jobs would exceed payloadSizeLimitBytes
func NewLeasePayloadLimit(numberOfJobsLimit int, payloadSizeLimitBytes int, maxJobSizeBytes int) LeasePayloadLimit {
	return LeasePayloadLimit{
		remainingJobCount:              numberOfJobsLimit,
		remainingPayloadSizeLimitBytes: payloadSizeLimitBytes,
		maximumJobSizeBytes:            maxJobSizeBytes,
	}
}
...
/*
 ...
 - remainingJobCount <= 0
 - remainingPayloadSizeLimitBytes <= maximumJobSizeBytes
...
func (s *LeasePayloadLimit) AtLimit() bool {
	return s.remainingJobCount <= 0 || s.remainingPayloadSizeLimitBytes <= s.maximumJobSizeBytes
}
