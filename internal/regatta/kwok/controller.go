...
const (
	ControllerImage = "registry.k8s.io/kwok/kwok:v0.7.0"
	ControllerName  = "armada-regatta-kwok-controller"
)

// controllerKubeconfigPath is where the internal kubeconfig bind-mounted into the
// kwok-controller container is written. Fixed (rather than a random temp file) so
// TeardownController can find and remove it - it must outlive RunController's own return,
// since deleting it too early can race the container's own open() of the bind-mounted file
// on VM-backed docker runtimes (observed with OrbStack) and crash the container silently.
var controllerKubeconfigPath = filepath.Join(os.TempDir(), "regatta-kwok-kubeconfig")
...
func RunController(ctx context.Context, kindClusterName string) error {
	out, err := exec.CommandContext(ctx, "docker", "ps",
		"--filter", "name=^/"+ControllerName+"$",
		"--format", "{{.Names}}",
	).Output()
	if err != nil {
		return fmt.Errorf("checking for existing kwok-controller: %w", err)
	}
	if strings.TrimSpace(string(out)) == ControllerName {
		return nil
	}
	...
	if err := os.WriteFile(controllerKubeconfigPath, internalKubeconfig, 0o600); err != nil {
	...
	cmd := exec.CommandContext(ctx, "docker",
		...
		"--name", ControllerName,
		"--network", "kind",
		"-v", controllerKubeconfigPath+":/kubeconfig:ro",
		ControllerImage,
...
func TeardownController(ctx context.Context) error {
	out, err := exec.CommandContext(ctx, "docker", "rm", "-f", ControllerName).CombinedOutput()
	if err != nil {
		return fmt.Errorf("stopping kwok-controller: %w: %s", err, out)
	}
	if err := os.Remove(controllerKubeconfigPath); err != nil && !os.IsNotExist(err) {
