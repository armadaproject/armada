...
func FromInternalCancel(userId string, queueName string, jobSetName string, time time.Time, e *armadaevents.CancelJob) ([]*api.EventMessage, error) {
	...
	return []*api.EventMessage{
		{
			Events: &api.EventMessage_Canceling{
				Canceling: &api.JobCancellingEvent{
					JobId:     jobId,
					JobSetId:  jobSetName,
					Queue:     queueName,
					Created:   time,
...
func FromInternalCancelled(userId string, queueName string, jobSetName string, time time.Time, e *armadaevents.CancelledJob) ([]*api.EventMessage, error) {
	...
	return []*api.EventMessage{
		{
			Events: &api.EventMessage_Canceled{
				Canceled: &api.JobCancelledEvent{
					JobId:     jobId,
					JobSetId:  jobSetName,
					Queue:     queueName,
					Created:   time,
...
func FromInternalResourceUtilisation(queueName string, time time.Time, e *armadaevents.ResourceUtilisation) ([]*api.EventMessage, error) {
	...
	return []*api.EventMessage{
		{
			Events: &api.EventMessage_Utilization{
				Utilization: apiEvent,
			},
		},
	}, nil
}
