...
SELECT j.job_id, j.queue, j.jobset, j.namespace, j.state, j.submitted, j.cancelled, j.cancel_reason, j.last_transition_time, j.latest_run_id, COALESCE(js.job_spec, j.job_spec) FROM job j join job_spec js on j.job_id = js.job_id WHERE job_id = ANY(sqlc.arg(job_ids)::text[]);
