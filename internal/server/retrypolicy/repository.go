...
// RetryPolicyExists reports whether a retry policy with the given name exists.
func (r *PostgresRetryPolicyRepository) RetryPolicyExists(ctx *armadacontext.Context, name string) (bool, error) {
	_, err := r.GetRetryPolicy(ctx, name)
	if err != nil {
		var notFound *ErrRetryPolicyNotFound
...
// Deleting a non-existent policy is a no-op, consistent with DeleteQueue.
