...
// OnTerminationMessage set but the compiled pattern nil (i.e. CompileRules
// was not called) does not vacuously match every error.
func TestMatchRule_UncompiledPatternFailsClosed(t *testing.T) {
	rule := &Rule{
		Action:               ActionFail,
		OnTerminationMessage: &errormatch.RegexMatcher{Pattern: ".*"},
	}
	assert.False(t, matchRule(rule, matchInput{terminationMessage: "anything"}),
