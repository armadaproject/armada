...
// TestMatchRule_UncompiledPatternFailsClosed ensures that a Rule with
// OnTerminationMessage set but compiledPattern nil (i.e. CompileRules was
// not called) does not vacuously match every error.
func TestMatchRule_UncompiledPatternFailsClosed(t *testing.T) {
	rule := &Rule{
		Action:               ActionFail,
		OnTerminationMessage: &errormatch.RegexMatcher{Pattern: ".*"},
	}
	// compiledPattern is deliberately nil: CompileRules was not called.
	assert.False(t, matchRule(rule, "", 0, "anything", nil),
