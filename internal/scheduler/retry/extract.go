...
// extractCategories returns the non-empty failure category and subcategory
// from the Error proto as a slice, preserving order (category first).
func extractCategories(err *armadaevents.Error) []string {
	if err == nil {
		return nil
	}
	var categories []string
	if c := err.GetFailureCategory(); c != "" {
		categories = append(categories, c)
	}
	if s := err.GetFailureSubcategory(); s != "" {
