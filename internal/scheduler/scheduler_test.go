...
var returnedOnceLeasedJob = testfixtures.JobDb.NewJob(
	...
	false,
	1,
).WithUpdatedRun(testfixtures.JobDb.CreateRun("testNodeId", &database.Run{
	RunID:               uuid.New(),
...
var (
	...
	requeuedJob         = testfixtures.JobDb.NewJob(
		...
		false,
		1,
	).WithUpdatedRun(testfixtures.JobDb.CreateRun("test-node", &database.Run{
		RunID:               uuid.New(),
		JobID:               requeuedJobId,
		JobSet:              "testJobSet",
		Executor:            "testExecutor",
		...
		"add job run": {
			...
			expectedUpdatedJobs: []*jobdb.Job{
				queuedJob.WithUpdatedRun(
					testfixtures.JobDb.CreateRun("test-executor-test-node", &database.Run{
						RunID:               uuid.UUID{},
						JobID:               queuedJob.Id(),
						JobSet:              queuedJob.Jobset(),
						Executor:            "test-executor",
						Node:                "test-node",
						Created:             123,
						ScheduledAtPriority: pointerFromValue(int32(5)),
