...
func JobRequirementsMet(node *internaltypes.Node, priority int32, jctx *schedulercontext.JobSchedulingContext) (bool, PodRequirementsNotMetReason, error) {
	return JobRequirementsMetForView(node, node.AllocatableAtPriority(priority), jctx)
}

// JobRequirementsMetForView is like JobRequirementsMet, but checks dynamic requirements against
// the given resource view instead of always reading node.AllocatableAtPriority. This lets urgency-based
// preemption pass node.AllocatableAtPriorityNoEviction, which excludes the fair-share eviction give-back.
...
func StaticJobRequirementsMet(node *internaltypes.Node, jctx *schedulercontext.JobSchedulingContext) (bool, PodRequirementsNotMetReason, error) {
	...
	matches, reason = HamiRequirementsMet(node, jctx)
	if !matches {
		return matches, reason, nil
	}
