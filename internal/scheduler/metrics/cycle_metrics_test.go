...
func TestReportPoolSchedulingOutcomes(t *testing.T) {
	...
	outcomes := []scheduling.PoolSchedulingOutcome{
		{
			Pool:              "pool-1",
			Success:           false,
			ErrorType:         scheduling.PoolSchedulingErrorTypeContextCreation,
	...
	errorCount := testutil.ToFloat64(m.poolSchedulingErrors.WithLabelValues("pool-1", string(scheduling.PoolSchedulingErrorTypeContextCreation)))
	assert.Equal(t, 1.0, errorCount, "context creation error metric does not match")
