...
type JobRepository interface {
	...
	FetchJobUpdates(ctx *context.ArmadaContext, jobSerial int64, jobRunSerial int64) ([]Job, []Run, error)
	...
	FetchJobRunErrors(ctx *context.ArmadaContext, runIds []uuid.UUID) (map[uuid.UUID]*armadaevents.Error, error)
	...
	CountReceivedPartitions(ctx *context.ArmadaContext, groupId uuid.UUID) (uint32, error)
	...
	FindInactiveRuns(ctx *context.ArmadaContext, runIds []uuid.UUID) ([]uuid.UUID, error)
	...
	FetchJobRunLeases(ctx *context.ArmadaContext, executor string, maxResults uint, excludedRunIds []uuid.UUID) ([]*JobRunLease, error)
}
...
// FetchJobRunErrors returns all armadaevents.JobRunErrors for the provided job run ids.  The returned map is
// keyed by job run id.  Any dbRuns which don't have errors wil be absent from the map.
func (r *PostgresJobRepository) FetchJobRunErrors(ctx *context.ArmadaContext, runIds []uuid.UUID) (map[uuid.UUID]*armadaevents.Error, error) {
	if len(runIds) == 0 {
...
// FetchJobUpdates returns all jobs and job dbRuns that have been updated after jobSerial and jobRunSerial respectively
// These updates are guaranteed to be consistent with each other
func (r *PostgresJobRepository) FetchJobUpdates(ctx *context.ArmadaContext, jobSerial int64, jobRunSerial int64) ([]Job, []Run, error) {
...
// FindInactiveRuns returns a slice containing all dbRuns that the scheduler does not currently consider active
// Runs are inactive if they don't exist or if they have succeeded, failed or been cancelled
func (r *PostgresJobRepository) FindInactiveRuns(ctx *context.ArmadaContext, runIds []uuid.UUID) ([]uuid.UUID, error) {
...
// FetchJobRunLeases fetches new job runs for a given executor.  A maximum of maxResults rows will be returned, while run
// in excludedRunIds will be excluded
func (r *PostgresJobRepository) FetchJobRunLeases(ctx *context.ArmadaContext, executor string, maxResults uint, excludedRunIds []uuid.UUID) ([]*JobRunLease, error) {
...
// to the provided groupId.  This is used by the scheduler to determine if the database represents the state of
// pulsar after a given point in time.
func (r *PostgresJobRepository) CountReceivedPartitions(ctx *context.ArmadaContext, groupId uuid.UUID) (uint32, error) {
	queries := New(r.db)
...
func insertRunIdsToTmpTable(ctx *context.ArmadaContext, tx pgx.Tx, runIds []uuid.UUID) (string, error) {
