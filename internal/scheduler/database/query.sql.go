...
UPDATE runs SET preempt_requested = true WHERE job_id = ANY($1::text[])
`

func (q *Queries) MarkJobRunsPreemptRequestedByJobId(ctx context.Context, jobIds []string) error {
	_, err := q.db.Exec(ctx, markJobRunsPreemptRequestedByJobId, jobIds)
	return err
}
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested FROM runs WHERE serial > $1 AND job_id = ANY($2::text[]) ORDER BY serial
`
