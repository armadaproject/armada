...
UPDATE runs SET preempt_requested = true, preempt_reason = $1, preempt_user = $2 WHERE queue = $3 and job_set = $4 and job_id = ANY($5::text[]) and terminated = false
`
...
func (q *Queries) MarkJobsCancelRequestedBySet(ctx context.Context, arg MarkJobsCancelRequestedBySetParams) error {
	_, err := q.db.Exec(ctx, markJobsCancelRequestedBySet,
		arg.CancelUser,
		arg.CancelReason,
		arg.JobSet,
		arg.Queue,
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, preempt_reason, preempt_user FROM runs WHERE serial > $1 AND job_id = ANY($3::text[]) ORDER BY serial LIMIT $2
`
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, preempt_reason, preempt_user FROM runs WHERE serial > $1 ORDER BY serial LIMIT $2
`
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, terminated, preempt_reason, preempt_user FROM runs WHERE serial > $1 AND job_id = ANY($2::text[]) ORDER BY serial
`
...
UPDATE jobs SET priority = $1, reprioritise_user = $2 WHERE queue = $3 and job_set = $4 and job_id = ANY($5::text[]) and terminated = false
`
...
UPDATE jobs SET priority = $1, reprioritise_user = $4 WHERE job_set = $2 and queue = $3 and terminated = false
`
