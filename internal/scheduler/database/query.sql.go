...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, gang_node_uniformity_label_value FROM runs WHERE serial > $1 AND job_id = ANY($3::text[]) ORDER BY serial LIMIT $2
`
...
func (q *Queries) SelectInitialRuns(ctx context.Context, arg SelectInitialRunsParams) ([]Run, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.Queue,
			&i.Pool,
			&i.GangNodeUniformityLabelValue,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, gang_node_uniformity_label_value FROM runs WHERE serial > $1 ORDER BY serial LIMIT $2
`
...
SELECT run_id, job_id, created, job_set, executor, node, cancelled, running, succeeded, failed, returned, run_attempted, serial, last_modified, leased_timestamp, pending_timestamp, running_timestamp, terminated_timestamp, scheduled_at_priority, preempted, pending, preempted_timestamp, pod_requirements_overlay, preempt_requested, queue, pool, gang_node_uniformity_label_value FROM runs WHERE serial > $1 AND job_id = ANY($2::text[]) ORDER BY serial
`
