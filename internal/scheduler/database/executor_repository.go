...
type ExecutorRepository interface {
	...
	GetExecutors(ctx *context.ArmadaContext) ([]*schedulerobjects.Executor, error)
	// GetLastUpdateTimes returns a map of executor name -> last heartbeat time
	GetLastUpdateTimes(ctx *context.ArmadaContext) (map[string]time.Time, error)
	// StoreExecutor persists the latest executor state
	StoreExecutor(ctx *context.ArmadaContext, executor *schedulerobjects.Executor) error
}
...
// GetExecutors returns all known executors, regardless of their last heartbeat time
func (r *PostgresExecutorRepository) GetExecutors(ctx *context.ArmadaContext) ([]*schedulerobjects.Executor, error) {
	queries := New(r.db)
...
// GetLastUpdateTimes returns a map of executor name -> last heartbeat time
func (r *PostgresExecutorRepository) GetLastUpdateTimes(ctx *context.ArmadaContext) (map[string]time.Time, error) {
	queries := New(r.db)
...
// StoreExecutor persists the latest executor state
func (r *PostgresExecutorRepository) StoreExecutor(ctx *context.ArmadaContext, executor *schedulerobjects.Executor) error {
	queries := New(r.db)
