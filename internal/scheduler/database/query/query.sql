...
UPDATE jobs
SET cancel_by_jobset_requested = true
WHERE job_set = sqlc.arg(job_set)
...
UPDATE jobs
SET succeeded = true
WHERE job_id = ANY (sqlc.arg(job_ids)::text[]);
...
FROM runs
WHERE serial > $1
  AND job_id = ANY (sqlc.arg(job_ids)::text[])
ORDER BY serial;
...
DELETE
FROM markers
WHERE created < sqlc.arg(cutoff)::timestamptz;
...
INSERT INTO markers (group_id, partition_id, created)
...
SELECT executor_id, last_updated
FROM executors;

-- name: UpsertExecutor :exec
INSERT INTO executors (executor_id, last_request, last_updated)
VALUES (sqlc.arg(executor_id)::text, sqlc.arg(last_request)::bytea,
        sqlc.arg(update_time)::timestamptz) ON CONFLICT (executor_id) DO
UPDATE SET (last_request, last_updated) = (excluded.last_request,excluded.last_updated);

-- name: SetLeasedTime :exec
UPDATE runs
SET leased_timestamp = $1
WHERE run_id = $2;

-- name: SetPendingTime :exec
UPDATE runs
SET pending_timestamp = $1
WHERE run_id = $2;

-- name: SetRunningTime :exec
UPDATE runs
SET running_timestamp = $1
WHERE run_id = $2;

-- name: SetTerminatedTime :exec
UPDATE runs
SET terminated_timestamp = $1
WHERE run_id = $2;
