...
SELECT job_id, job_set, queue, priority, submitted, queued, queued_version, validated, cancel_requested, cancel_user, cancel_by_jobset_requested, cancelled, succeeded, failed, scheduling_info, scheduling_info_version, pools, price_band, serial, preempt_user, reprioritize_user FROM jobs WHERE serial > $1 AND terminated = false ORDER BY serial LIMIT $2;

-- name: SelectUpdatedJobs :many
SELECT job_id, job_set, queue, priority, submitted, queued, queued_version, validated, cancel_requested, cancel_user, cancel_by_jobset_requested, cancelled, succeeded, failed, scheduling_info, scheduling_info_version, pools, price_band, serial, preempt_user, reprioritize_user FROM jobs WHERE serial > $1 ORDER BY serial LIMIT $2;
...
UPDATE jobs SET cancel_by_jobset_requested = true, cancel_user = COALESCE(cancel_user, $1) WHERE job_set = sqlc.arg(job_set) and queue = sqlc.arg(queue) and queued = ANY(sqlc.arg(queued_states)::bool[]) and terminated = false;

-- name: MarkJobsCancelRequestedBySet :exec
UPDATE jobs SET cancel_by_jobset_requested = true, cancel_user = COALESCE(cancel_user, $1) WHERE job_set = sqlc.arg(job_set) and queue = sqlc.arg(queue) and terminated = false;
...
UPDATE jobs SET cancel_requested = true, cancel_user = COALESCE(cancel_user, $1) WHERE queue = sqlc.arg(queue) and job_set = sqlc.arg(job_set) and job_id = ANY(sqlc.arg(job_ids)::text[]) and terminated = false;
...
UPDATE jobs SET priority = $1, reprioritize_user = sqlc.arg(reprioritize_user) WHERE queue = sqlc.arg(queue) and job_set = sqlc.arg(job_set) and job_id = ANY(sqlc.arg(job_ids)::text[]) and terminated = false;
