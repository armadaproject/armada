...
// NewServer creates a new api lookout server but does not configure it
func NewServer(api *operations.LookoutAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/lookout.sock"`
...
func (s *Server) SetAPI(api *operations.LookoutAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
