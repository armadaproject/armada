...
type GetJobsRepository interface {
	GetJobs(ctx *context.ArmadaContext, filters []*model.Filter, order *model.Order, skip int, take int) (*GetJobsResult, error)
}
...
func (r *SqlGetJobsRepository) GetJobs(ctx *context.ArmadaContext, filters []*model.Filter, activeJobSets bool, order *model.Order, skip int, take int) (*GetJobsResult, error) {
	var jobRows []*jobRow
	var runRows []*runRow
...
func makeJobRows(ctx *context.ArmadaContext, tx pgx.Tx, tmpTableName string) ([]*jobRow, error) {
	query := fmt.Sprintf(`
		SELECT
			j.job_id,
			j.queue,
...
func makeRunRows(ctx *context.ArmadaContext, tx pgx.Tx, tmpTableName string) ([]*runRow, error) {
	query := fmt.Sprintf(`
		SELECT
		    jr.job_id,
			jr.run_id,
...
func makeAnnotationRows(ctx *context.ArmadaContext, tx pgx.Tx, tempTableName string) ([]*annotationRow, error) {
	query := fmt.Sprintf(`
		SELECT
			ual.job_id,
			ual.key,
