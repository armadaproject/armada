...
func (qb *QueryBuilder) JobCount(filters []*model.Filter) (*Query, error) {
	...
	tablesFromColumns, err := qb.getAllTables(normalFilters, &model.Order{})
	if err != nil {
		return nil, err
	}
...
func (qb *QueryBuilder) InsertIntoTempTable(tempTableName string, filters []*model.Filter, order *model.Order, skip, take int) (*Query, error) {
	...
	tablesFromColumns, err := qb.getAllTables(normalFilters, order)
	if err != nil {
		return nil, err
	}
	queryTables, err := qb.determineTablesForQuery(tablesFromColumns)
...
func (qb *QueryBuilder) getAllTables(filters []*model.Filter, order *model.Order) ([]map[string]bool, error) {
	...
	for _, filter := range filters {
		// Ignore annotation columns, we deal with annotation filters separately
