...
func (qb *QueryBuilder) GroupByJsonb(
	...
	if groupedField.IsAnnotation {
		groupByColumn = annotationColumn(groupedField.Field)
	} else {
	...
	agggregates, err := qb.getAggregatesSql(queryAggregators)
	if err != nil {
		return nil, err
	}
	...
	query := fmt.Sprintf(`SELECT %s.%s, %s
		...
		%s
		%s`,
		jobTableAbbrev, groupByColumn, agggregates,
		jobTable, jobTableAbbrev,
		activeJobSetsFilter,
		where,
...
func (qb *QueryBuilder) makeWhereClause(filter *model.Filter) (string, error) {
	...
	if filter.IsAnnotation {
		...
		case model.MatchExact:
			i := qb.recordValue(map[string]interface{}{filter.Field: filter.Value})
			...
			return fmt.Sprintf("%s.annotations @> %s", jobTableAbbrev, i), nil
		case model.MatchExists:
			i := qb.recordValue(filter.Field)
			return fmt.Sprintf("%s.annotations ? %s", jobTableAbbrev, i), nil
		default:
			column = annotationColumn(filter.Field)
		}
	} else {
		var err error
		column, err = qb.lookoutTables.ColumnFromField(filter.Field)
...
func annotationColumn(key string) string {
	return fmt.Sprintf("annotations->>'%s'", strings.ReplaceAll(key, "'", "''"))
}
...
func (qb *QueryBuilder) valueForMatch(value interface{}, match string) (string, error) {
	...
	case model.MatchAnyOf:
		switch v := value.(type) {
		case []int:
			return fmt.Sprintf("ANY (%s)", qb.recordValue(v)), nil
		default:
