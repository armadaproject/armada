...
func buildRule(cfg CategoryRule) (rule, error) {
	...
	for _, cond := range cfg.OnConditions {
		if !errormatch.KnownConditions[cond] {
			return rule{}, fmt.Errorf("unknown condition %q, valid values: %s, %s, %s, %s",
				cond, errormatch.ConditionOOMKilled, errormatch.ConditionEvicted, errormatch.ConditionDeadlineExceeded, errormatch.ConditionAppError)
		}
	}
...
func matchesCondition(conditions []string, containers []containerInfo, podReason string) bool {
	for _, cond := range conditions {
		...
		case errormatch.ConditionOOMKilled:
			for _, c := range containers {
				if c.reason == errormatch.ConditionOOMKilled {
					return true
				}
			}
		case errormatch.ConditionAppError:
			for _, c := range containers {
				if c.reason == errormatch.ConditionAppError {
					return true
				}
			}
		case errormatch.ConditionEvicted:
			if podReason == errormatch.ConditionEvicted {
				return true
			}
		case errormatch.ConditionDeadlineExceeded:
