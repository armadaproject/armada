...
func StartUp(config configuration.ExecutorConfiguration) (func(), *sync.WaitGroup) {
	wg := &sync.WaitGroup{}
	wg.Add(1)
	return executor.StartUpWithContext(config, context.NewFakeClusterContext(config.Application.ClusterId), nil, task.NewBackgroundTaskManager(metrics.ArmadaExecutorMetricsPrefix), wg)
}
