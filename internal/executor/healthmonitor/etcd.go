...
// Run the service until ctx is canceled.
...
func (srv *EtcdHealthMonitor) getInstanceCurrentFractionOfResourceInUse(instance *etcdInstanceMonitor) (float64, error) {
	...
	if instance.lastCheck.IsZero() {
		return 0, fmt.Errorf("no scrape has ever occurred for this instance, possibly etcd health check is still initializing")
	}
