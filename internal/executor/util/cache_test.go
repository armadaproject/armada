...
func TestMapPodCache_Add_Expires(t *testing.T) {
	pod := makeManagedPod("job1")
	cache := NewMapPodCacheWithExpiry(time.Second/10, time.Second/100, "metric2")
	cache.Add(pod)
