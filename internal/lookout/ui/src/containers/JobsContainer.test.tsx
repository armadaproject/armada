import { JobFilters, updateColumnsFromQueryString, makeQueryString } from "./JobsContainer";
...
describe("makeFiltersFromQueryString", () => {
  test("empty string returns default filters", () => {
    const query = ""
    const filters = updateColumnsFromQueryString(query)
  ...
  test.each(orderingsCases)("makes filter with ordering %p", (query, expectedOrdering) => {
    const filters = updateColumnsFromQueryString(query as string)
    expect(filters).toStrictEqual({
      queue: "",
      jobSet: "",
      jobStates: [],
