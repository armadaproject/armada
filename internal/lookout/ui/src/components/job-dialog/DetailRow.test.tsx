...
function SetUpReactTable(name: string, value: string, annotation?: string) {
  return (
    <TableContainer>
      <Table className="details-table-container">
        <TableBody>
          <DetailRow key={name} annotation={annotation} detailRowKey={name} name={name} value={value} />
        </TableBody>
      </Table>
    </TableContainer>
  )
...
describe("DetailRow", () => {
  it("DetailRow with no links in annotations", async () => {
    const actual = SetUpReactTable("detail", "NOTURL", "NOTURL")
    render(actual)
  ...
  it("DetailRow with links in annotations", async () => {
    const actual = SetUpReactTable("detail", "http://google.org", "http://google.org")
    render(actual)
  ...
  it("DetailRow With Bad Link in annotation", async () => {
    const actual = SetUpReactTable("detail", "//google.org", "//google.org")
    render(actual)
    const linkRole = await screen.queryByRole("//google.org")
    expect(linkRole).toBeFalsy()
    expect(screen.getByText("//google.org")).toBeInTheDocument()
