  ...
  if (allDefined(props)) {
    content = (
      <table>
        ...
        <tr>
          <td className="cell-left">{props.firstQuartile && <div>first quartile: {secondsToDurationString(props.firstQuartile)}</div>}</td>
          <td className="cell-right">{props.thirdQuartile && <div>third quartile: {secondsToDurationString(props.thirdQuartile)}</div>}</td>
        </tr>
        <tr>
          <td className="cell-left">{props.median && <div>median: {secondsToDurationString(props.median)}</div>}</td>
          <td className="cell-right">{props.average && <div>average: {secondsToDurationString(props.average)}</div>}</td>
        </tr>
        </tbody>
      </table>
    )
