...
func (s *Scanner) ScanAll(ctx context.Context) ([]StreamInfo, error) {
	...
	for {
		...
		cmd := s.client.Scan(ctx, cursor, constants.EventStreamPrefix+"*", s.config.ScanBatchSize)
...
func (s *Scanner) processBatch(ctx context.Context, keys []string) ([]StreamInfo, error) {
	...
	// Partition keys into pipeline batches
	batches := armadaSlices.PartitionToMaxLen(keys, s.config.PipelineBatchSize)
	...
	for i, batch := range batches {
		...
		// Sleep between batches if configured and not the last batch
		if s.config.InterBatchDelay > 0 && i < len(batches)-1 {
			select {
			case <-ctx.Done():
				return nil, ctx.Err()
			case <-time.After(s.config.InterBatchDelay):
