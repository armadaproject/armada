...
// The jobSetId may contain ':' characters, so we split on the first ':' after the prefix.
func parseStreamKey(key string) (queue, jobSetId string, err error) {
	// Check if the key has the correct prefix
	if !strings.HasPrefix(key, constants.EventStreamPrefix) {
		return "", "", fmt.Errorf("stream key does not have %q prefix: %s", constants.EventStreamPrefix, key)
	}

	// Strip the prefix
	rest := strings.TrimPrefix(key, constants.EventStreamPrefix)

	// Split on the first ':' to separate queue from jobSetId
	// This handles jobSetId values that contain ':' characters
