...
func TestSetNoopOTelShutsDownPreviousProvider(t *testing.T) {
	processor := &shutdownObservingSpanProcessor{shutdown: make(chan struct{})}
	...
	case <-time.After(time.Second):
		t.Fatal("setNoopOTel did not shut down the previous tracer provider")
	}
	assert.IsType(t, tracenoop.TracerProvider{}, otel.GetTracerProvider())
}

type shutdownObservingSpanProcessor struct {
...
func (p *shutdownObservingSpanProcessor) Shutdown(ctx context.Context) error {
	close(p.shutdown)
	return nil
}

func (p *shutdownObservingSpanProcessor) ForceFlush(ctx context.Context) error {
	return nil
}
...
func TestSamplerMustBeConfigured(t *testing.T) {
	cfg := ObservabilityConfig{
		Exporter: OTLPExporterConfig{
		...
		Resource: ResourceAttributes{
			ServiceName:     "server",
			ServiceVersion:  "1.0.0",
			ServiceInstance: "test-sampler-required",
		},
	}
...
func TestSamplerOverrideByConfig(t *testing.T) {
	tests := []struct {
		...
		{name: "parent_based_trace_id_ratio", sampler: testParentBasedTraceIDRatioSampler, samplerArg: 0.25, wantArg: 0.25},
	}
...
func TestSamplingRatioEnforcesValidRange(t *testing.T) {
	tests := []struct {
		...
		{name: "parent_based_trace_id_ratio below 0", sampler: testParentBasedTraceIDRatioSampler, samplerArg: -0.5, shouldFail: true},
		{name: "parent_based_trace_id_ratio above 1", sampler: testParentBasedTraceIDRatioSampler, samplerArg: 2.0, shouldFail: true},
