...
func MessageUnmarshaller(ctx *armadacontext.Context, msg pulsar.ConsumerMessage, metrics *commonmetrics.Metrics) *utils.EventsWithIds[*controlplaneevents.Event] {
	...
	if err != nil {
		...
		ctx.Logger.WithError(err).Warnf("Could not unmarshal proto for msg %s", msg.ID())
	} else {
...
func BatchMetricPublisher(ctx *armadacontext.Context, metrics *commonmetrics.Metrics, batch *utils.EventsWithIds[*controlplaneevents.Event]) {
	...
	ctx.Infof("Batch being processed contains a ControlPlane event of type %v and associated message ID", countOfEventsByType)
}
