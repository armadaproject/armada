...
COPY "cmd/eventingester/" "cmd/eventingester/"
COPY "internal/" "internal/"
COPY "pkg/" "pkg/"
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    CGO_ENABLED=0 go build -gcflags="all=-N -l" -o eventingester "./cmd/eventingester"
...
COPY --from=builder /app/eventingester /app/eventingester
