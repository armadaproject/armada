...
scheduler: ${GO_BIN:-go} build -gcflags="all=-N -l" -o ./dist/armada-scheduler ./cmd/scheduler/main.go && ./dist/armada-scheduler run --config ./_local/scheduler/config-regatta.yaml
...
executor: export KUBECONFIG=.kube/external/config-regatta-1 && ${GO_BIN:-go} build -gcflags="all=-N -l" -o ./dist/armada-executor ./cmd/executor/main.go && ./dist/armada-executor --config ./_local/executor/config-kwok.yaml
executor2: export KUBECONFIG=.kube/external/config-regatta-2 && ${GO_BIN:-go} build -gcflags="all=-N -l" -o ./dist/armada-executor2 ./cmd/executor/main.go && ./dist/armada-executor2 --config ./_local/executor/config-kwok-2.yaml
