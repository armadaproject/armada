...
lookoutui: sh -c 'cd internal/lookoutui && yarn install && yarn run openapi && VITE_PORT=3000 PROXY_TARGET=http://localhost:8089 yarn dev'
...
lookouthcui: sh -c 'until [ -f internal/lookoutui/src/openapi/armada/index.ts ]; do echo "Waiting for openapi generation..."; sleep 2; done && cd internal/lookoutui && VITE_PORT=3001 PROXY_TARGET=http://localhost:8091 VITE_CACHE_DIR=node_modules/.vite-hc yarn dev'
