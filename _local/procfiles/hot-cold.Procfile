...
lookoutui: sh -c 'cd internal/lookoutui && touch .openapi-in-progress && rm -f .openapi-ready && yarn install && yarn run openapi && rm -f .openapi-in-progress && touch .openapi-ready && VITE_PORT=3000 PROXY_TARGET=http://localhost:8089 yarn dev'
...
lookouthcui: sh -c 'until [ -f internal/lookoutui/.openapi-ready ] && [ ! -f internal/lookoutui/.openapi-in-progress ]; do echo "Waiting for openapi generation..."; sleep 2; done && cd internal/lookoutui && VITE_PORT=3001 PROXY_TARGET=http://localhost:8091 VITE_CACHE_DIR=node_modules/.vite-hc yarn dev'
