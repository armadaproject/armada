...
COPY "cmd/lookoutingester/" "cmd/lookoutingester/"
COPY "internal/" "internal/"
COPY "pkg/" "pkg/"
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
    CGO_ENABLED=0 go build -gcflags="all=-N -l" -o lookoutingester "./cmd/lookoutingester"
...
COPY --from=builder /app/lookoutingester /app/lookoutingester
...
ENTRYPOINT ["/app/lookoutingester"]
