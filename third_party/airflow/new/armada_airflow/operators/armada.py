...
from armada_airflow.client.armada_client import ArmadaClient
from armada_airflow.logs.token_retriever import TokenRetriever
from armada_airflow.logs.pod_log_manager import PodLogManager
...
class ArmadaOperator(BaseOperator, LoggingMixin):
    ...
    def _poll_for_termination(self, tracking_message: str) -> None:
        ...
        while armada_client.is_active(state):
            state = self.client.get_job_status(self.job_id)
            self.log.info(
                f"job {self.job_id} is in state: {JobState.Name(state)}. {tracking_message}"
            )
        ...
        self.log.info(
            f"job {self.job_id} terminated with state: {JobState.Name(state)}"
        )
        if state != JobState.SUCCEEDED:
            raise AirflowException(
                f"job {self.job_id} did not succeed. Final status was {JobState.Name(state)}"
            )
