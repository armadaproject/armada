...
def test_success_job(client: ArmadaClient, context: Any, channel_args:
GrpcChannelArgs, mocker):
  ...
  mocker.patch("armada.operators.armada.ArmadaOperator._reattach_or_submit_job", return_value=job_id)
...
def test_bad_job(client: ArmadaClient, context: Any, channel_args:
GrpcChannelArgs, mocker):
  job_set_name = f"test-{uuid.uuid1()}"
  job = client.submit_jobs(
    queue=DEFAULT_QUEUE,
  ...
  try:
    operator.execute(context)
    pytest.fail("Operator did not raise AirflowException on job failure as expected")
  except AirflowException as _: # Expected
    response = operator.client.get_job_status([job_id])
...
def success_job(task_number: int, context: Any, channel_args: GrpcChannelArgs) -> (
  JobState):
  operator = ArmadaOperator(
...
def test_parallel_execution(client: ArmadaClient, context: Any, channel_args: GrpcChannelArgs,
                            mocker):
  threads = []
  success_job(task_number=0, context=context, channel_args=channel_args)
  for task_number in range(5):
    t = threading.Thread(target=success_job, args=[task_number, context, channel_args])
    t.start()
    threads.append(t)
...
@pytest.mark.skip(reason="we should not test performance in the CI.")
def test_parallel_execution_large(client: ArmadaClient, context: Any, channel_args: GrpcChannelArgs,
                                  mocker):
  threads = []
  success_job(task_number=0, context=context, channel_args=channel_args)
...
@pytest.mark.skip(reason="we should not test performance in the CI.")
def test_parallel_execution_huge(client: ArmadaClient, context: Any, channel_args: GrpcChannelArgs,
                                 mocker):
  threads = []
  success_job(task_number=0, context=context, channel_args=channel_args)
