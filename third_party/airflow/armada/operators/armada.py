...
class _ArmadaClientFactory:
    ...
    _clients: Dict[str, ArmadaClient] = {}

    @staticmethod
    def client_for(args: GrpcChannelArgs) -> ArmadaClient:
        channel_args_key = str(args.serialize())
        try:
            _ArmadaClientFactory._lock.acquire()
            if channel_args_key not in _ArmadaClientFactory._clients:
                _ArmadaClientFactory._clients[channel_args_key] = ArmadaClient(
                    channel=args.channel()
                )
            return _ArmadaClientFactory._clients[channel_args_key]
        finally:
            _ArmadaClientFactory._lock.release()
...
"""
    ...
    def execute(self, context) -> None:
        ...
        self.job_context = _RunningJobContext(
            self.armada_queue, self.job_set_id, job_id
        )
        if self.deferrable:
    ...
    def on_kill(self) -> None:
        if self.job_context is not None:
            self.log.info(
                f"on_kill called, cancelling job with id {self.job_context.job_id} in queue "
    ...
    def _reattach_or_submit_job(
        ...
        if existing_id is not None:
            self.log.info(
                f"Attached to existing job with id {existing_id['armada_job_id']}. {self._trigger_tracking_message(existing_id['armada_job_id'])}"
            )
    ...
    def _running_job_terminated(self, context: _RunningJobContext):
        ...
        if context.state != JobState.SUCCEEDED:
            raise AirflowException(
                f"job {context.job_id} did not succeed. Final status was {context.state.name}"
            )
    ...
    def _check_job_status_and_fetch_logs(
        ...
        if state != context.state:
            self.log.info(
                f"job {context.job_id} is in state: {state.name}. {self._trigger_tracking_message(context.job_id)}"
            )
        context.state = state

        if context.state == JobState.UNKNOWN:
            if (
                DateTime.now().diff(context.start_time).in_seconds()
                > self.job_acknowledgement_timeout
            ):
                self.log.info(
                    f"Job {context.job_id} not acknowledged by the Armada server within "
