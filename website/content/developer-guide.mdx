...
This project has a [Dev Container](https://code.visualstudio.com/docs/devcontainers/containers) that includes all the tools required for Armada development. To configure Git within the devcontainer, create an environment file at `.devcontainer/devcontainer.env`:

```
# example devcontainer.env
GIT_USER_NAME=Hugh.Man
GIT_USER_EMAIL=hugh.man@example.com
GIT_SIGNING_KEY=ssh-123 somethingforwarded
```

The `GIT_SIGNING_KEY` is can be set to your [SSH signing key](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent).
