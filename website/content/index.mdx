---
title: Overview
description: System overview of Armada, a multi-Kubernetes cluster batch job scheduler.
---
...
- metadata specific to Armada (e.g., the name of the queue to which the job belongs).
...
- The Armada control-plane components are responsible for job submission and scheduling. Jobs are submitted to the control plane and are automatically transferred to worker clusters once scheduled.
- The Armada executors are responsible for communication between the Armada control plane and the [Kubernetes control plane](https://kubernetes.io/docs/concepts/overview/kubernetes-api/). There is one executor per worker cluster.
...
We show an overview of an Armada deployment with three worker clusters below. Clients here refer to systems submitting jobs to Armada.
...
The arrows in the above diagram show the path jobs take through the system. Specifically, each job passes through the following stages:

1. The job is submitted to Armada, where it is stored in one of several queues, e.g., corresponding to different users. At this point, the job is stored on the Armada control plane and does not exist in any Kubernetes cluster. The job is in the _queued_ state.
2. The Armada scheduler eventually determines that the job should be run and marks the job with the Kubernetes cluster and node it should be run on. The job is in the _leased_ state.
...
4. Container images are pulled, volumes and secrets mounted, and init containers run, after which the main containers of the pod start running. The job is in the _running_ state.
5. The job terminates once all containers in the pod have stopped running. Depending on the exit codes of the containers, the job transitions to either the _succeeded_ or _failed_ state.
...
- Job submission and control. Exposes an API through which clients can submit, re-prioritize, and cancel jobs.
...
graph LR
    subgraph P [Pulsar]
        T[Multiple topics/topic partitions]
    end
