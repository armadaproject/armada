...
You can optionally set up Armada to route all job events through a persistent NATS Streaming subject before saving them to Redis. This is useful if an additional application needs to consume events from Armada as NATS subject contains job events from all job sets.
...
You can also set the location of the config file using the command line argument:
...
For Open ID protected server armadactl will perform PKCE flow opening a web browser.
...
For Kerberos authentication, the config file should contain this:
...
For more information about usage please see the [User Guide](./user.mdx)
...
jobs:
  - priority: 0
    podSpec:
      ...
      restartPolicy: Never
      containers: ... any Kubernetes pod spec ...
```
